...
Knowing the correct names is important for precision in conversations about networking — a router forwards *packets*, a switch forwards *frames*, and TCP deals in *segments*.
...
SOP prevents a malicious page at `evil.com` from reading the cookies or response body from `bank.com`. JavaScript can *send* cross-origin requests (with CORS) but cannot *read* the response without explicit permission.
